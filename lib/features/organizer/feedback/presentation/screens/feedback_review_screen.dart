...
class _FeedbackReviewScreenState extends State<FeedbackReviewScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_feedbacks.isNotEmpty) {
      avgRating = _feedbacks.map((e) => e.ratingOverall).reduce((a, b) => a + b) / _feedbacks.length;
    }

    return Scaffold(
      backgroundColor: const Color(0xFFF9FAFB),
      appBar: AppBar(
        title: const Text('Event Feedback', style: TextStyle(color: Colors.black)),
            ...
            child: Row(
              children: [
                Column(
                  ...
                  children: [
                    Text(avgRating.toStringAsFixed(1), style: TextStyle(fontSize: 48.sp, fontWeight: FontWeight.bold, color: AppColors.primary)),
                    RatingBarIndicator(
                      rating: avgRating,
                    ...
                    SizedBox(height: 4.h),
                    Text('${_feedbacks.length} Reviews', style: TextStyle(color: Colors.grey, fontSize: 14.sp)),
                  ],
                ),
                const Spacer(),
      ...
      children: [
        ...
        Container(
          ...
          child: FractionallySizedBox(
            ...
            widthFactor: pct,
            child: Container(decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.circular(3))),
          ),
        ),
      ],
    );
